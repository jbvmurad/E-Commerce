...
export function SellerStub({ title, description = 'Bu sayfa yakında kullanıma açılacaktır.' }: { title: string; description?: string }) {
  return (
    <div style={{ fontFamily: 'DM Sans, sans-serif' }} className="space-y-5">
      <h1 style={{ color: SELLER_ACCENT, fontFamily: 'Playfair Display, serif', fontSize: 22, letterSpacing: '0.04em', textShadow: `0 0 12px ${SELLER_ACCENT}50` }}>{title.toUpperCase()}</h1>
      <div className="flex flex-col items-center justify-center py-24 rounded"
        ...
        <div className="w-12 h-12 rounded-full flex items-center justify-center mb-4"
          ...
          <Construction size={22} style={{ color: SELLER_ACCENT, filter: `drop-shadow(0 0 6px ${SELLER_ACCENT}80)` }} />
        </div>
