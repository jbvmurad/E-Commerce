...
export function ConfirmEmail() {
  ...
  useEffect(() => {
    ...
    if (!code) {
      setError('Email confirmation code is missing.');
      return;
    }

    authService.confirmEmail(code)
      .then((response) => setMessage(response.message))
      .catch((requestError) => setError(requestError instanceof ApiError ? requestError.message : 'Email doğrulanamadı.'));
  }, [code]);
      ...
      <div className="relative mx-auto w-full max-w-2xl border border-[rgba(0,245,255,0.28)] bg-[rgba(5,13,21,0.94)] p-7 shadow-[0_0_55px_rgba(0,245,255,0.14)] sm:p-10">
        ...
        <div className="text-center">
          <div className="mb-3 flex items-center justify-center gap-2 text-xs font-semibold uppercase tracking-[0.28em] text-[#7cecf2]">
            <ShieldCheck size={15} /> Secure account verification
          </div>
          <h1 className="font-display text-3xl text-[#00f5ff] sm:text-4xl" style={{ textShadow: '0 0 22px rgba(0,245,255,0.72)' }}>
            {pending ? 'VERIFYING YOUR EMAIL' : message ? 'EMAIL VERIFIED' : 'VERIFICATION FAILED'}
          </h1>
          <p className="mx-auto mt-5 max-w-lg text-sm leading-7 text-muted-foreground">
            {pending && 'Your secure confirmation link is being checked. Please keep this page open.'}
            {message && message}
            {error && error}
          </p>
        </div>
        ...
        <div className="mt-8 border border-[#00f5ff]/12 bg-[#02070c] p-4 text-sm text-[#8db8c2]">
          <div className="flex items-start gap-3">
            ...
            <p>After verification, use your confirmed email and password to access your E-Commerce account.</p>
          </div>
        </div>

        {!pending && (
          <div className="mt-8 flex justify-center">
            <Link to="/login" className="inline-flex items-center gap-2 rounded-lg bg-gradient-to-r from-[#00f5ff] to-[#00b4c8] px-6 py-3 font-bold text-[#020408] shadow-[0_0_24px_rgba(0,245,255,0.34)] transition-transform hover:scale-[1.03]">
